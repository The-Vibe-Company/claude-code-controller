...
describe("notification-sender", () => {
  describe("sendNotification", () => {
    it("sends Slack notification with correct URL and body", async () => {
      ...
      expect(callArgs[0]).toBe("https://hooks.slack.com/services/test");
    ...
    it("sends Telegram notification to correct API", async () => {
      ...
      expect(result.success).toBe(true);
      const callArgs = mockFetch.mock.calls[0];
      expect(callArgs[0]).toBe("https://api.telegram.org/bot123:ABC/sendMessage");
      expect(callArgs[1].method).toBe("POST");
      expect(callArgs[1].signal).toBeDefined();
      const body = JSON.parse(callArgs[1].body);
    ...
    it("sends Resend notification with auth header", async () => {
      ...
      expect(callArgs[1].headers.Authorization).toBe("Bearer re_test123");
      expect(callArgs[1].signal).toBeDefined();
      const body = JSON.parse(callArgs[1].body);
    ...
    it("sends Gotify notification with X-Gotify-Key header", async () => {
      ...
      expect(result.success).toBe(true);
      const callArgs = mockFetch.mock.calls[0];
      expect(callArgs[0]).toBe("https://gotify.example.com/message");
      expect(callArgs[1].headers["X-Gotify-Key"]).toBe("gotify-token");
      expect(callArgs[1].signal).toBeDefined();
      const body = JSON.parse(callArgs[1].body);
    ...
    it("sends ntfy notification to topic URL with headers", async () => {
      ...
      expect(result.success).toBe(true);
      const callArgs = mockFetch.mock.calls[0];
      expect(callArgs[0]).toBe("https://ntfy.sh/companion-alerts");
      expect(callArgs[1].headers.Authorization).toBe("Bearer ntfy-token");
      expect(callArgs[1].headers.Priority).toBe("4");
      expect(callArgs[1].headers.Title).toBe("Companion");
      expect(callArgs[1].signal).toBeDefined();
    });
